...
public class DatabaseExampleActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mDbHelper = new DatabaseOpenHelper(this);// this is a subclass of SQLiteOpenHelper
		...
		// Create a cursor
		Cursor c = readArtists(); //is an iterator over serveral records request in a sql operation
		mAdapter = new SimpleCursorAdapter(this, R.layout.list_layout, c,
				DatabaseOpenHelper.columns, new int[] { R.id._id, R.id.name },
				0);

		setListAdapter(mAdapter);//display adapter that display the adapter on the display.
